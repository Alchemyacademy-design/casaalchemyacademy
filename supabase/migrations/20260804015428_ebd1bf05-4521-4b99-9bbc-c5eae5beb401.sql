create or replace function public.internal_apply_stripe_invoice_paid(
  ...
  p_plan_key public.membership_plan_key, p_course_id bigint, p_stripe_subscription_id text,
  p_stripe_customer_id text, p_stripe_price_id text, p_subscription_status text,
  p_current_period_start timestamptz, p_current_period_end timestamptz,
  p_cancel_at_period_end boolean, p_stripe_invoice_id text, p_stripe_payment_intent_id text,
  p_amount integer, p_currency text, p_livemode boolean, p_metadata jsonb default '{}'::jsonb
) returns jsonb
language plpgsql
security definer
set search_path to ''
as $function$
begin
  -- Idempotency by Stripe event id: a redelivered event must not re-apply
  -- membership / entitlement changes.
  if private.stripe_event_already_applied(p_stripe_event_id, p_stripe_subscription_id, null) then
    return jsonb_build_object('result', 'processed_ignored_duplicate', 'membership_id', null);
  end if;

  return private.apply_stripe_invoice_paid(
