CREATE OR REPLACE FUNCTION public.get_alchemist_leaderboard(p_window text DEFAULT 'all'::text, p_limit integer DEFAULT 10)
 RETURNS TABLE(user_id uuid, display_name text, avatar_path text, xp integer, tier text, rank integer)
 LANGUAGE plpgsql
  ...
  WITH
    ...
    ranked AS (
      SELECT
        ...
        t.total_xp,
        CASE
          WHEN t.total_xp >= 4000 THEN 'Philosopher''s Stone'
          WHEN t.total_xp >= 1500 THEN 'Mastery'
          WHEN t.total_xp >= 500 THEN 'Refinement'
          WHEN t.total_xp >= 100 THEN 'Awakening'
          ELSE 'Prima Materia'
        END AS tier_name,
...
BEGIN
  ...
  IF v_xp >= 4000 THEN
    v_tier := 'Philosopher''s Stone'; v_next_tier := NULL; v_next_at := NULL; v_current_floor := 4000;
  ELSIF v_xp >= 1500 THEN
    v_tier := 'Mastery'; v_next_tier := 'Philosopher''s Stone'; v_next_at := 4000; v_current_floor := 1500;
  ELSIF v_xp >= 500 THEN
    v_tier := 'Refinement'; v_next_tier := 'Mastery'; v_next_at := 1500; v_current_floor := 500;
  ELSIF v_xp >= 100 THEN
    v_tier := 'Awakening'; v_next_tier := 'Refinement'; v_next_at := 500; v_current_floor := 100;
  ELSE
    v_tier := 'Prima Materia'; v_next_tier := 'Awakening'; v_next_at := 100; v_current_floor := 0;
  END IF;

  SELECT leaderboard_opt_out INTO v_opt_out FROM public.profiles WHERE id = v_uid;

  SELECT lb.rank INTO v_rank_all
  FROM public.get_alchemist_leaderboard('all', 500) lb
  WHERE lb.user_id = v_uid;

  SELECT lb.rank INTO v_rank_30d
  FROM public.get_alchemist_leaderboard('30d', 500) lb
  WHERE lb.user_id = v_uid;
