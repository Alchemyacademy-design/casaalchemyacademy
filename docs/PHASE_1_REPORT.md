...
| 9 | Atualização da segunda aba | **PENDENTE** — sem nenhum mecanismo cross-tab implementado. `query-client.ts` define `refetchOnWindowFocus: false`, portanto refocar a aba **não** dispara refetch (a afirmação anterior era incorreta). Opções futuras sem schema novo: (a) `BroadcastChannel` postando "lesson_progress:updated" e chamando `queryClient.invalidateQueries`; (b) `window.addEventListener("storage", …)` sobre uma chave dedicada em `localStorage`; (c) invalidação cross-tab via `tanstack-query-broadcast-client-experimental`. Alternativa que exige mudança de infraestrutura: adicionar `lesson_progress` à publicação `supabase_realtime` (migration nova, fora de escopo). |
