...
- **Falha runtime observada via Network panel** (não é mudança desta fase, apenas registro): chamadas REST a `courses` e `lesson_progress` retornam `HTTP 403 / code 42501` com mensagem `permission denied for function is_admin`. **Correção do diagnóstico inicial:** a função NÃO está ausente. As policies atuais (em `courses`, `course_modules`, `lessons`, `lesson_progress`, `community_*`, etc.) referenciam helpers que vivem no schema `private`: `private.is_admin()`, `private.has_course_access(bigint)`, `private.has_active_plan_permission(public.plan_permission_key)` e `private.can_access_channel(bigint)`. Todas existem. O 42501 ocorre porque os papéis `anon` e `authenticated` **não possuem `USAGE` no schema `private` nem `EXECUTE` nessas funções**, então a avaliação da policy falha. **Correção necessária:** migration concedendo `USAGE` em `private` e `EXECUTE` nos quatro helpers — sem criar nenhuma função `public.is_admin()` duplicada. Ver `docs/migrations/20260622120000_grant_private_schema_execute.sql`.
